...
def load_data():
    print('loading data!')
    data_mat = h5py.File(r'D:\单细胞\code\文献代码\scDCC-master\data\Small_Datasets\10X_PBMC_top2000.h5')
    ...
    print(len(set(Y)))
    print(Y)
    return adata.X,Y, adata.raw.X, adata.obs.size_factors


class AAE_net(nn.Module):
    def __init__(self,X_dim, N,M,z_dim,n_clusters,sigma=0.,alpha=1.,gamma=1.):
        super().__init__()
        self.alpha = alpha
        self.sigma = sigma
        self.gamma = gamma
    ...
    def forward(self,x):
        z_noise  = self.Q_net(x+torch.randn_like(x) * self.sigma)
        z_noise_2 = self.P_net(z_noise)
    ...
    #对于自编码器进行预训练
    def pretrian(self,x, X_raw, size_factor,epoch = 300,batch_size=256,lr=0.001):   
        self.train()
    ...
    def fit(self,x,y,X_raw, sf,lr=0.1,lr_d=0.005 ,batch_size=256, num_epochs=100,save_dir=""):#10X数据参数,也可64
        y-=1
...
if __name__ == '__main__':
    ...
    np.save(r'C:\Users\lenovo\Desktop\hxb\10X_label.npy',y_pred)#用np.load读取，读取为一个数组
